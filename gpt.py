...
def add_user_chat(c, conn, name, chat):
    conn.execute("INSERT INTO history (name, input) VALUES (?, ?)", (name, chat))
    conn.commit()

def add_sys_chat(c, conn, name, chat):
    conn.execute("INSERT INTO history (name, input) VALUES (?, ?)", (name, chat))
    conn.commit()
...
def ask_gpt(message):
    ...
    recent.extend(get_recent_chats(c, conn, message.author.id))
    add_user_chat(c, conn, message.author.id, message.content)
    ...
    add_sys_chat(c, conn, message.author.id, text)
    return text 
